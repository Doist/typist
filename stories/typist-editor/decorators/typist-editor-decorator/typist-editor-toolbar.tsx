...
function TypistEditorToolbar({ editor }: TypistEditorToolbarProps) {
    // The editor state lives outside React, so we use `useSyncExternalStore` to re-render this
    // component on selection and content changes, keeping the toolbar buttons `pressed` state
    // in sync with the current selection and applied marks
    const editorState = useSyncExternalStore(
        function subscribeToEditorState(callback) {
    ...
    const buttonConfigs = useMemo<ToolbarButtonConfig[]>(
        () => [
            {
                'aria-label': 'Bold',
        ...
        [editor, editorState, handleImageButtonClick, handleLinkButtonClick, isCursorOverLink],
    )
